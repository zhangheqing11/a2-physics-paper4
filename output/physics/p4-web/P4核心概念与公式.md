...
本文按 Cambridge 9702 2025至2027年考纲第12至25章整理，仅覆盖 Paper 4 的 A2 理论，并补充必要的 AS 接口知识。它与《P4 学习与题型全册》配套：本文查“是什么、为什么、何时能用”，题型全册查“题目如何识别和求解”。

尚未学过 A2 时，每章先读概念，再读公式的条件，最后到题型全册看代表题并独立做变式。能背出公式不等于掌握；至少应能说明每个量、为什么能用，以及条件改变后哪里需要重建模型。

本文的“核心”指考纲明确要求的概念和关系；“推导”指从核心关系得到的常用结论；“接口”指所需 AS 或先修知识。公式标签不代表考试是否提供该式，考试中仍应阅读该卷数据表。英文定义是可用于学习和作答的物理表述，不是逐题评分标准的替代品。


## 阅读顺序与导航

先读通用工具，再按圆周与引力、温度与气体与热力学、振动、电场与电容、磁场与感应与交流、量子与核物理、医学与天体的顺序学习。各章标题采用官方编号，可通过 Markdown 标题导航。
...
合力决定加速度；后一式给平均合力，瞬时关系取变化率。惯性质量不变的经典力学模型。
...
仅适用于加速度恒定的同一方向分量，不能直接用于方向不断改变的圆周速度。
...
θ为恒力与位移夹角；P式给平均功率。速率接近光速时不能使用经典动能。
...
前式要求g近似恒定；后式要求取力沿速度的分量，适用于瞬时机械功率。
...
电荷Q(C)，电流I(A)，电势差V(V)，电阻R(Ω)；Q=It要求电流恒定。V=W/Q表示每库仑转移的能量。
...
n载流子数密度(m⁻³)，A截面积(m²)，q电荷量大小(C)，v_d漂移速率(m s⁻¹)；波速=频率(Hz)×波长(m)。
...
中间式为平均角速率；匀速时也是任意时刻角速率。一周为2π rad，f=1/T。
...
同一角速率下半径越大，线速率越大。比较加速度时须先说清固定的是v还是ω。


**核心**

$$
\sum F_{r} = \frac{mv^{2}}{r} = mr\omega ^{2}
$$

以指向圆心为径向正方向；ΣF_r是径向合力，不一定等于某一单独的力。
...
给出大小。点质量适用；均匀球体外部可等效为球心点质量，r=R+h，不是h。
...
$$
U = m\varphi  = -\frac{GMm}{r}
$$

无穷远取零；U是两质量系统的引力势能。向外移动时势能增加，虽仍可能为负。


**推导**

$$
v_{\mathrm{orbit}} = \sqrt{\frac{GM}{r}}
$$
...
圆轨道且引力独自提供向心力。轨道半径增大，速率减小而周期增大。
...
只适用于上述圆轨道；总能量为动能加引力势能，不是势能本身。


**推导**

$$
\Delta U = GMm(\frac{1}{r_{1}}-\frac{1}{r_{2}})
$$
...
逃逸速率假设出发后无推进、无阻力，刚好到无穷远时速率为零。不是圆轨道速率。


**推导**

$$
g_{r} = -\frac{d\varphi }{dr}
$$
...
r向外为正，所以g_r为负。能量式要求没有其他非保守功；多源时势代数叠加、场矢量叠加。
...
后式指温差的数值：升高1 K与升高1°C相同。温度的比值和气体方程必须用K。
...
第一式用于无相变升降温且c近似恒定；第二式用于相变。混合过程分阶段相加。
...
P heater power 加热功率(W)，t时间(s)；恒功率且全部计入输入能量。容器吸热、散热都要另计。


**推导**

$$
Q_{\mathrm{released}} = Q_{\mathrm{absorbed}}
$$

$$
\theta  = 100\frac{X-X_{0}}{X_{100}-X_{0}}
$$

热平衡能量式须包含各物体及损失。后式只适用于按0°C和100°C固定点线性校准的测温性质X。
...
n按摩尔计，N按粒子个数计，两套计数不可混用。m=M/N_A。
...
$$
n=\frac{m_{\mathrm{gas}}}{M}
$$

前式要求气体数量不变；漏气或分装时应分别追踪N或n，不能自动约掉。
...
均方速率为所有分子速率平方的平均值。1/3来自三维各方向均等。


**核心**

$$
\langle E_{k}\rangle=\frac{1}{2}m\langle c^{2}\rangle=\frac{3}{2}kT
$$

单分子的平均平动动能只由T决定；同温不同气体平均平动动能相同，速率一般不同。
...
速率正比于温度平方根、反比于分子质量平方根。气体质量密度ρ=Nm/V。
...
$$
\Delta U=Q+W_{\mathrm{on}}=Q-W_{\mathrm{by}}
$$

吸热Q正、放热Q负；压缩功W_on正、膨胀功W_on负。始终保持同一约定。


**核心**

$$
W_{\mathrm{by}}=p\Delta V
$$

$$
W_{\mathrm{on}}=-p\Delta V
$$

外压恒定。准静态过程可用气体p-V图；变压时功取有向曲线下的面积。
...
用于仅计平动动能的理想单原子气体模型，且第二式n不变。不能当作所有多原子气体的通用内能公式。
...
负号为恢复方向；a-x图是过原点的负斜率直线，斜率=-ω²。


**核心**

$$
x=x_{0}\sin (\omega t+\varphi )
$$

$$
v=\omega x_{0}\cos (\omega t+\varphi )
$$

初相位由起始位置和速度决定；不能看到SHM就默认t=0时x=0。
...
$$
v_{\mathrm{max}}=\omega x_{0}
$$

$$
a_{\mathrm{max}}=\omega ^{2}x_{0}
$$

同一位移可能有两个速度方向；最大加速度大小发生于两端。
...
$$
K=\frac{1}{2}m\omega ^{2}(x_{0}^{2}-x^{2})
$$

U以平衡位置为零；竖直弹簧需把弹簧势能与重力势能共同计入恢复势能。
...
相位差用同频运动的时间错位。弹簧周期要求线性胡克定律，k_s弹簧劲度系数(N m⁻¹)。
...
单摆长度l(m)，g重力加速度；仅小角度、轻绳、可视作质点且阻尼可忽略，不是任意摆幅通式。
...
$$
F_{\mathrm{magnitude}}=\frac{|Qq|}{4\pi \varepsilon _{0}r^{2}}
$$

第一式须按方向理解；后一式为力的大小，方向另由电荷正负与几何判断。
...
径向向外为正；Q为带符号源电荷。势以无穷远为零，单源点电荷或球外适用。


**核心**

$$
U=qV=\frac{Qq}{4\pi \varepsilon _{0}r}
$$
...
电势能包含被放入的电荷q；E沿电势下降最快方向。V-x图负斜率对应正向电场。
...
$$
W_{\mathrm{external}}=q\Delta V
$$

第一式要求只有电力做功；第二式是缓慢移动、动能不变时外力做功。负电荷向高电势运动时动能可增加。
...
$$
y=\frac{1}{2}\frac{qE}{m}(\frac{l}{u})^{2}
$$

平行板粒子偏转：初速度u平行板面，板长l，初横向速度零、忽略其他力，y为有符号偏移。
...
$$
C_{\mathrm{parallel}}=C_{1}+C_{2}+\cdots 
$$

并联各支路电势差相同，各电容板上电荷大小相加得到对应总电荷。


**核心**

$$
\frac{1}{C_{\mathrm{series}}}=\frac{1}{C_{1}}+\frac{1}{C_{2}}+\cdots 
$$

串联总电势差相加；常用等电荷条件来自初始中性且与外界绝缘的中间节点。
...
准静态充电从0到末态，V-Q图下方面积给能量。最后一个等式由Q=CV推导。
...
通过恒定电阻R放电，C也恒定，不存在外部继续充电源。


**核心**

$$
I_{\mathrm{mag}}=\frac{V_{0}}{R}e^{-t/(RC)}
$$

给放电电流大小。若把流入正板为正，电流为dQ/dt，放电时带负号。


**推导**

$$
\ln (\frac{V}{V_{0}})=-\frac{t}{RC}
$$

$$
t_{\mathrm{half}}=RC \ln 2
$$

对数图斜率=-1/(RC)；电荷、电压、电流大小有相同半衰时间。
...
$$
t_{\mathrm{half,energy}}=\frac{RC \ln 2}{2}
$$

储能正比于V²，因此能量衰减更快，不能用Q的半衰时间代替。


**推导**

$$
C_{\mathrm{sphere}}=4\pi \varepsilon _{0}r
$$

$$
C_{\mathrm{plates}}=\frac{\varepsilon A}{d}
$$

球式由球面电势推导；板式为常用扩展关系，要求匀强场、忽略边缘效应，ε介质介电常量，A正对面积。
...
匀强场、速度垂直B、无其他力且经典非相对论。磁力只改变方向，动能不变。
...
速度选择器：电场力与磁力大小相等、方向相反。只是筛选能直行的速率，不是把所有粒子加速到此值。
...
大小式；载流子正负决定哪一侧电势高。电流沿片长，B沿厚度t，电压跨宽w。
...
霍尔推导取力的大小。消去E_H、v_d、w，得到上式；不要把宽w误代为厚t。
...
匀强场、平面线圈；α是与法线而非与平面夹角。场垂直圈面时磁通最大。
...
$$
\varepsilon _{\mathrm{average}}=-\frac{\Delta \Lambda }{\Delta t}
$$

负号表示楞次方向约定；只求大小则取变化率绝对值。磁通很大但不变时，ε仍为零。
...
导体杆的有效l、速度v、B相互垂直；电流式要求回路闭合且总电阻为R，忽略其他电动势。


**推导**

$$
\Lambda =NBA \cos \omega t
$$
...
匀速转动的理想线圈，t=0时法线平行B，磁场均匀。峰值ε₀=NBAω。
...
只适用于完整正弦交流；不是所有波形通式。正弦周期平均电流为零，但有效值非零。
...
$$
\overline{P}=I_{\mathrm{rms}}^{2}R=\frac{V_{\mathrm{rms}}^{2}}{R}=\frac{P_{\mathrm{max}}}{2}
$$

最后一个等式要求正弦纯电阻；瞬时功率始终非负，变化频率为交流频率的两倍。


**推导**

$$
I_{\mathrm{rms}}=\sqrt{\langle I^{2}\rangle}
$$

任何波形先平方、对时间平均、再开方；非正弦要按实际波形计算。


**推导**

$$
V_{\mathrm{rms,half}}=\frac{V_{0}}{2}
$$

$$
\overline{P}_{\mathrm{half}}=\frac{V_{0}^{2}}{4R}
$$

理想无平滑半波整流；平方非零时间只有原来的一半，不能仍除以√2。


**推导**

$$
V_{\mathrm{rms,full}}=\frac{V_{0}}{\sqrt{2}}
$$

$$
f_{\mathrm{ripple,full}}=2f
$$

理想无平滑全波整流。半波峰间隔1/f，全波峰间隔1/(2f)。


**推导**

$$
RC \gg \Delta t_{\mathrm{peaks}}
$$

$$
\Delta V \approx \frac{I_{\mathrm{load}}\Delta t_{\mathrm{peaks}}}{C}
$$

前式为小纹波的常用条件，后式假设负载电流近似恒定且纹波较小；不是任何整流波形的精确式。
...
真空光；光子无静止质量但有能量和动量，不能用mv并取m=0断言光子动量为零。
...
$$
hf=\Phi +K_{\mathrm{max}}
$$

爱因斯坦光电方程为能量守恒；同一方程各能量先统一J或eV。


**核心**

$$
K_{\mathrm{max}}=\frac{1}{2}m_{e}v_{\mathrm{max}}^{2}
$$

$$
\Phi =hf_{0}=\frac{hc}{\lambda _{0}}
$$

f₀阈频、λ₀阈波长；经典电子动能式适用于非相对论速率。
...
$$
\dot{N}_{\gamma }=\frac{P}{hf}
$$

$$
I_{\mathrm{photo}}=e\dot{N}_{e}
$$

P入射功率(W)，光子率和收集电子率(s⁻¹)。若量子效率不足1或收集不全，电子率小于光子率。
...
前式为德布罗意关系；后式取正能级差，对发射和匹配吸收都适用。


**推导**

$$
\lambda =\frac{h}{\sqrt{2m|q|V_{\mathrm{acc}}}}
$$

带电粒子从近似静止经加速电压V_acc获得动能，忽略损失且非相对论。
...
$$
\Delta m=Zm_{p}+(A-Z)m_{n}-m_{\mathrm{nucleus}}
$$
...
m_p、m_n分别为质子、中子质量。若用原子质量须一致处理电子，不能随意混核质量和原子质量。


**核心**

$$
E_{\mathrm{released}}=(\sum m_{\mathrm{before}}-\sum m_{\mathrm{after}})c^{2}
$$

反应物减产物；质量减少才放能。所有反应产物包括自由中子等都要计入。


**推导**

$$
E_{\mathrm{released}}=\sum B_{\mathrm{products}}-\sum B_{\mathrm{reactants}}
$$

$$
1u c^{2}\approx931.5 \,\mathrm{MeV}
$$

结合能方法以核子构成总数匹配为前提；给每核子结合能时先乘相应核子数，再作差。


**核心**

$$
A_{\mathrm{act}}=\lambda N
$$

$$
\lambda =\frac{\ln 2}{t_{\mathrm{half}}}
$$

活度为正的衰变率大小。若t用天，λ必须按天⁻¹；计算Bq时转成秒⁻¹。
...
$$
A_{\mathrm{act}}=A_{0}e^{-\lambda t}
$$

单一核素且无额外生成或混入。指数函数描述统计平均趋势，不是逐秒精确计数。


**推导**

$$
\frac{N}{N_{0}}=(\frac{1}{2})^{t/t_{\mathrm{half}}}
$$

$$
t=\frac{\ln (N_{0}/N)}{\lambda }
$$

衰变掉的核数为N₀−N，不是N；多种独立核素的总活度需逐项相加。


**推导**

$$
C_{\mathrm{net}}=C_{\mathrm{measured}}-C_{\mathrm{background}}
$$

$$
C_{\mathrm{net}}=\eta A_{\mathrm{act}}
$$

C计数率(s⁻¹)，η综合探测比例；几何、效率等不变时净计数率随活度同比衰减，不必数值相等。
...
$$
t=2\sum \frac{d_{i}}{c_{i}}
$$

单一均匀介质用前式，分层介质用后式；t是去和回的总时间。
...
$$
R_{I}=\frac{I_{R}}{I_{0}}=(\frac{Z_{2}-Z_{1}}{Z_{2}+Z_{1}})^{2}
$$

法向入射的强度反射系数，范围0到1；别当作振幅反射系数。无界面吸收时透射比例为1−R_I。
...
$$
\frac{I}{I_{0}}=e^{-\sum \mu _{i}x_{i}}
$$

第一式适用于均匀材料；后式为多层单程传播，不包含另外的界面反射损失。μ与x须用匹配单位。


**推导**

$$
x_{\mathrm{half}}=\frac{\ln 2}{\mu }
$$

$$
\frac{I_{\mathrm{echo}}}{I_{\mathrm{incident}}}=R_{I}e^{-2\mu d}
$$

回波式是假设单界面、同一均匀介质且不再计其他透射损失的简化模型；往返衰减路程为2d。


**核心**

$$
E_{\gamma ,max}=eV_{\mathrm{acc}}
$$

$$
\lambda _{\mathrm{min}}=\frac{hc}{eV_{\mathrm{acc}}}
$$

一个电子把全部获得的动能转成一个X光子时达到极限，实际多数光子能量更低。


**核心**

$$
E_{\gamma }=m_{e}c^{2}\approx0.511 \,\mathrm{MeV}
$$

近静止电子与正电子湮灭的两个光子各有此能量；总能量为1.022 MeV，非单个光子能量。


**推导**

$$
\lambda _{\gamma }=\frac{h}{m_{e}c}
$$
...
波长式用上述近静止湮灭光子。时间差式给相对两探测器中点的位移大小，假设探测路径为同一直线。
...
假设各向同性辐射，且吸收可忽略或已校正。距离加倍，接收强度变四分之一。
...
恒星近似黑体，λ_max为按波长绘制的谱峰。r是恒星半径，不是到观察者的距离d。
...
$$
\frac{L_{1}}{L_{2}}=(\frac{r_{1}}{r_{2}})^{2}(\frac{T_{1}}{T_{2}})^{4}
$$

同模型下做比值可减少常量计算；固定光度时更热的恒星半径更小。


**核心**

$$
z=\frac{\lambda _{\mathrm{obs}}-\lambda _{0}}{\lambda _{0}} \approx \frac{v}{c}
$$

z为红移，无量纲；λ₀实验室或发射波长。低速线性近似，不能用于任意高红移精确宇宙学。


**核心**

$$
\frac{f_{0}-f_{\mathrm{obs}}}{f_{0}} \approx \frac{v}{c}
$$
...
频率式写的是减少量的正大小。哈勃关系用于大尺度退行趋势，近邻星系的自身运动可能显著。
...
哈勃时间给时间尺度；把它直接当宇宙精确年龄还需假设，实际膨胀历史可能变化。
...
$$
x_{\mathrm{half}}=\frac{\ln 2}{k}
$$

统一数学形式。此处k只是衰减比例常量，不是玻尔兹曼常量；x可为时间或距离，单位随场景改变。
...
主要依据为 Cambridge International AS and A Level Physics 9702 syllabus for 2025 2026 and 2027，官方正文第26至39页。第12至25章的所有小节均已作为本文覆盖核对项；章首列出对应编号。AS接口与标为“推导”的内容服务于核心模型理解，不代表每一条都是考纲独立列出的背诵要求。

官方考纲 https://www.cambridgeinternational.org/Images/664565-2025-2027-syllabus.pdf

配套资料为本项目《CIE9702 P4 学习与题型全册 2022至2025》。本文负责概念和公式，真题小问、代表题解与精确配对的评分标准索引见题型全册。医学章节仅讨论考试中的物理原理，不构成诊断或治疗建议。
